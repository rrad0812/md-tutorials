...
from .third_party.werkzeug._compat import iteritems, text_type, integer_types, string_types, to_bytes, to_unicode
...
class SQL(object):
    ...
    def _to_bool(value):
        ...
        if isinstance(value, str):
            ...
            if v in ('0', 'false', 'f', 'no', 'n', 'off', ''):
                return False
            try:
                # ako je string broj, tretiraj 0 kao False
                return float(v) != 0.0
            except Exception:
                return True
        # fallback
    ...
    def _convert_field_value(self, field, value, filter_type, db_module):
        ...
        if filter_type and filter_type in [consts.FILTER_CONTAINS, consts.FILTER_STARTWITH, consts.FILTER_ENDWITH]:
            ...
            if data_type == consts.DATE:
                ...
                ############################################################################
                if db_module.DATABASE == 'POSTGRESQL':
                    return self._to_bool(field.data)
                elif value:
                    return '1'
                else:
                    return '0'
    ...
    def _get_condition(self, field, filter_type, value, db_module):
        ...
        if field.data_type == consts.BOOLEAN:
            ...
            # Patch for Postgresql and boolean field type
            ############################################################################
            if ((db_module == 'POSTGRESQL' and self._to_bool(field.data) == False) or (value == '0')):
                if filter_sign == '=':
